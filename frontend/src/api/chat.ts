...
function getDemoReply(text: string): string {
  ...
  if (lower.includes("hello") || lower.includes("hi")) {
    return "Good evening. Tyler interface is operating in workshop demo mode.";
  }
...
export async function sendChatMessage(sessionId: string, text: string): Promise<ChatResponse> {
  ...
  try {
    ...
    throw new Error("Tyler service is unavailable. Please try again shortly.");
  }

  if (!response.ok) {
    const body: { detail?: string } = await response.json().catch(() => ({}));
    throw new Error(body.detail ?? "Tyler could not process that message.");
  }
